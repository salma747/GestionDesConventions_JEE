...
public interface ConventionRepository extends JpaRepository<Convention,Long> {
    @Query(value = "select * from convention where type=:type ",nativeQuery = true)
    List<Convention> findByType(@Param("type") long type);
    @Query(value = "select * from convention  where objet like %:objet% ",nativeQuery = true)
    List<Convention> findByObjet( @Param("objet") String objet);
    @Query(value = "select * from convention  where type=:type and objet like %:objet% ",nativeQuery = true)
